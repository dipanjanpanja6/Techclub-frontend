...
const ShowCase = (props) => {
    ...
    //     )
    // }) : ''
    const allProject = state ? state.map(p => {
        return (
            <div key={p.id}>
                <ListItem dense>
                    <ListItemIcon>
                    ...
                    <ListItemText primary={p.title} disableTypography secondary={
                        // <React.Fragment>
                        <div>
    ...
    const topProject = top ? top.map(p => {
        return (
            <ShowCard auth={props.auth} key={p.id} id={p.id} email={p.email} open={p.status} Title={p.title} image={p.imageUri} new={(new Date(Date.parse(p.createdAt)) - new Date()) <= 14 ? true : false} Details={p.desc} />
        )
    }) : <CircularProgress style={{margin:12}}/>
