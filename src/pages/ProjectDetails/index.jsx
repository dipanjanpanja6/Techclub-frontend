...
function ProjectDetailsPage(props) {
  ...
  const tag = state
    ? state.tag.map((p, i) => {
        return <Chip key={i} style={{ margin: 7, backgroundColor: "#0F1" }} label={"#" + p} size="small" />
      })
    : ""
  return (
    <Card className={classes.root}>
      <CardActionArea onClick={fabSwitch}>
        <CardMedia className={classes.media} image={state.imageUri} title="Contemplative Reptile" />
        <Grid style={{ position: "absolute", top: 60, left: 12 }}>
          {new Date(Date.parse(state.createdAt)) - new Date() <= 14 ? <Chip style={{ marginRight: 7 }} color="secondary" label="NEW" size="small" /> : ""}

          {state.status && <Chip style={{ backgroundColor: "#ff0" }} label="Member Require" size="small" />}
      ...
      <CardContent>
        ...
        <Typography style={{ maxHeight: 300, overflow: "hidden" }} variant="body2" color="textSecondary" component="p">
          Github Link :{" "}
          {
            <Link rel="external nofollow noopener noreferrer" target="_blank" variant="caption" color="primary" href={state.gitLink}>
              {state.gitLink}
            </Link>
          }
        </Typography>
        <Typography style={{ maxHeight: 300, overflow: "hidden" }} variant="body2" color="textSecondary" component="p">
          Project Live at{" "}
          {
            <Link rel="external nofollow noopener noreferrer" target="_blank" variant="caption" color="primary" href={state.liveLink}>
              {state.liveLink}
            </Link>
          }
        </Typography>
