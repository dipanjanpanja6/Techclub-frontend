...
import { Typography, Grid, Fab, Divider, TextField, InputAdornment,ListItemSecondaryAction, Link, ListItem, ListItemText, List, ListItemIcon, Switch, Chip, IconButton } from '@material-ui/core';
...
function ProjectSubmit(props) {
    ...
    const Prj = projects ? projects.map(p => {
        return (
            <ListItem key={p.id} disabled={!p.active} button onClick={() => { history.push(`/project/${p.id}`) }}>
                <ListItemIcon>
                    <FolderSharedIcon />
                </ListItemIcon>
    ...
    const tag = state.tag.map((p, i) => {
        return (
            <Chip style={{ margin: '0 1px' }} size='small' key={i} label={p} onDelete={() => {
                console.log("dlt");
                setState({
                    ...state, tag:
                        state.tag.filter(i => i !== p)
    ...
    return (
        <Grid item className={sty.root}>
            ...
            <br />
             ...
             <form onKeyPress={(e)=>{if (e.keyCode === 13 || e.which === 13) {
                e.preventDefault();
                return false;
            }}} className={sty.from} onSubmit={submit}>
                ...
                <br />
                 ...
                 onChange={handelChangeS}  value={Stag} label="Tag for your project" helperText='Maximum 3 tag or your project will not approve'
                    InputProps={{
                        startAdornment: (
            ...
            <Grid style={{ display: 'inline-block' }}>
                <Switch checked={state.status} onChange={handleCheck} /><Typography variant='caption'>Project is in initial stage. Member require ?</Typography>
            </Grid>
            <br />
